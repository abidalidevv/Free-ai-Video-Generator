...
function CreatePage() {
  ...
  const [subject, setSubject] = useState(() => state.currentProject?.subject || state.currentProject?.name || "History of AI");
  const [language, setLanguage] = useState(() => state.currentProject?.script?.language || "auto");
  const [paragraphs, setParagraphs] = useState(() => state.currentProject?.script?.paragraphs || 4);
  ...
  const [script, setScript] = useState(() => state.currentProject?.script?.text || DEFAULT_SCRIPT);
  const [keywords, setKeywords] = useState<string[]>(() => {
    if (state.currentProject?.keywords && state.currentProject.keywords.length > 0) {
      return state.currentProject.keywords.map(k => k.text);
    }
  ...
  /* auto save to active project and projectStorageService */
  useEffect(() => {
    const t = setTimeout(() => {
      setSavedAt(new Date());
      const baseProject = state.currentProject || {
        id: crypto.randomUUID(),
        name: subject || "Untitled Project",
        aspectRatio: "9:16",
      ...
      const updated = {
        ...baseProject,
        name: subject || baseProject.name || "Untitled Project",
        subject: subject,
        updatedAt: new Date().toISOString(),
        script: {
          text: script,
        ...
        keywords: keywords.map((k, i) => ({ id: `k-${i}`, text: k })),
      };
      dispatch({ type: "SAVE_PROJECT", payload: updated });
    }, 800);
    return () => clearTimeout(t);
  }, [script, keywords, subject, language, paragraphs]);
  ...
  const startGenerate = async (keywordsOnly = false) => {
    ...
    try {
      if (!keywordsOnly) {
        setSteps({ subject: "done", thinking: "running", script: "idle", keywords: "idle", done: "idle" });
        // Real MPT POST /api/v1/scripts
        setSteps(s => ({ ...s, thinking: "done", script: "running" }));
        const scriptRes = await llmService.generateScript({
        ...
        const generatedScript = scriptRes.script || (scriptRes as any).video_script;
        if (generatedScript) {
          pushHistory(generatedScript);
        }
        setSteps(s => ({ ...s, script: "done", keywords: "running" }));
        // Real MPT POST /api/v1/terms
        const termsRes = await llmService.generateTerms({
          script: generatedScript || script,
          subject: subject.trim(),
          amount: 8,
        });
        const generatedTerms = termsRes.terms || (termsRes as any).video_terms;
        if (generatedTerms && generatedTerms.length > 0) {
          setKeywords(generatedTerms);
        }
        setSteps(s => ({ ...s, keywords: "done", done: "done" }));
        toast.success("Script and keywords generated from MoneyPrinterTurbo!");
      } else {
        setSteps({ subject: "done", thinking: "idle", script: "done", keywords: "running", done: "idle" });
        const termsRes = await llmService.generateTerms({
          script,
          subject: subject.trim(),
          amount: 8,
        ...
        setSteps(s => ({ ...s, keywords: "done", done: "done" }));
        toast.success("Keywords generated from MoneyPrinterTurbo!");
      }
    } catch (err: any) {
      console.error("MPT LLM error:", err);
      toast.error("Generation failed: " + (err.message || "Cannot reach MoneyPrinterTurbo backend"));
      setSteps(s => ({ ...s, script: "failed", keywords: "failed" }));
    } finally {
      setGenerating(false);
    }
  };
    ...
    <AppShell>
      <PageHeader
        ...
        actions={<>
          ...
          <GhostButton onClick={() => { setSavedAt(new Date()); toast.success("Draft saved successfully!"); }}><Save className="w-4 h-4" /> Save Draft</GhostButton>
          <PrimaryButton onClick={() => navigate({ to: "/render" })}>Continue to Render Studio <ArrowRight className="w-4 h-4" /></PrimaryButton>
        </>}
      />
      ...
      <div className="grid grid-cols-12 gap-5 mt-5">
        ...
        <div className="col-span-12 xl:col-span-8 space-y-5">
          ...
          >
            <KeywordsManager keywords={keywords} setKeywords={setKeywords} onRegenerate={() => startGenerate(true)} />
          </SectionCard>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <PrimaryButton onClick={() => navigate({ to: "/video-settings" })} className="!h-12 !px-7 !text-[14px]">
...
function KeywordsManager({ keywords, setKeywords, onRegenerate }: { keywords: string[]; setKeywords: (k: string[]) => void; onRegenerate?: () => void }) {
  ...
  return (
    <div className="pt-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <PrimaryButton onClick={onRegenerate}><RefreshCw className="w-4 h-4" /> Regenerate</PrimaryButton>
      </div>
